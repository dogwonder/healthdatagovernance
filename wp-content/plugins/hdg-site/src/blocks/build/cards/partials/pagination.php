...
if ( $total_pages > 1 ) :
?>
<nav class="sigur-pagination border-top" aria-label="Pagination">
<ul class="sigur-pagination__list">
 ...
 if ($total_pages > 5):
   ...
   ?>
		<li class="sigur-pagination__item">
			<a href="<?php echo get_pagenum_link(
     '1'
   ); ?>" class="sigur-pagination__link">1</a>
		</li>
		<?php
  if ($paged > 3) {
    echo '<span>...</span>';
  ...
  for ($i = $start_page; $i <= $end_page; $i++) { ?>
			<li class="sigur-pagination__item<?php echo $paged === $i
     ? ' sigur-pagination__item--active'
     : ''; ?>">
				<a href="<?php echo get_pagenum_link($i); ?>" class="sigur-pagination__link">
					<?php echo $i; ?>
				</a>
			</li>
			<?php }
  if ($paged < $total_pages - 2) {
    echo '<span class="sigur-pagination__seperator">...</span>';
  }
  ?>
		<li class="sigur-pagination__item">
			<a href="<?php echo get_pagenum_link(
     $total_pages
   ); ?>" class="sigur-pagination__link">
