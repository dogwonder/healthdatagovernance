...
class GP_Unique_ID extends GP_Plugin {
	...
	/**
	 * An array of all unique IDs generated by GPUID during the current runtime.
	 *
	 * This is a safeguard for snippets (or 3rd-parties) that use the gp_unique_id()->get_unique() method to generate IDs.
	 *
	 * @var array $_generated_uids_safeguard An array of generated IDs.
	 */
	public static $_generated_uids_safeguard = array();
	...
	public function tooltips( $tooltips ) {
		...
		$tooltips[ $this->perk->key( 'prefix' ) ]          = sprintf( $template, __( 'Unique ID Prefix', 'gp-unique-id' ), sprintf( __( 'Prepend a short string to the beginning of the generated ID (i.e. %1$sabc%2$s123890678).', 'gp-unique-id' ), '<span style="background:rgba( 255, 255, 255, 0.3 ); padding:0 0.1rem; font-weight:bold;">', '</span>' ) );
		$tooltips[ $this->perk->key( 'suffix' ) ]          = sprintf( $template, __( 'Unique ID Suffix', 'gp-unique-id' ), sprintf( __( 'Append a short string to the end of the generated ID (i.e. 123890678%1$sxyz%2$s).', 'gp-unique-id' ), '<span style="background:rgba( 255, 255, 255, 0.3 ); padding:0 0.1rem; font-weight:bold;">', '</span>' ) );
	...
	public function get_unique( $form_id, $field, $length = 5, $atts = array(), $entry = false, $check_spam = true ) {
		...
		/**
		 * Let's assume that any entry with a transaction ID is not spam – an easy way for us to avoid having to bypass
		 * Gravity Forms honeypot validation when generating a unique ID in post-submission contexts (e.g. waiting for
		 * payment). If we run into future issues with this, it'd be worth exploring whether we can check the `status` of
		 * the entry rather than calling the GFCommon::is_spam_entry() method which has been problematic in the past.
		 */
		$is_spam = $entry && ! rgar( $entry, 'transaction_id' ) && GFCommon::is_spam_entry( $entry, $form );
		...
		self::$_generated_uids_safeguard[] = $unique;
